...
void initialise_paging()
{
    ...
    u32int mem_end_page = 0x10000000; //0x1000000
	...
	puts("Cloning kernel_directory\n");
	current_directory = clone_directory(kernel_directory);
	puts("Switching to cloned directory...\n");
	switch_page_directory(current_directory);
	puts("Exiting intialise_paging...\n");
}
...
void switch_page_directory(page_directory_t *dir)
{
	...
	puts("switch_page_directory before enabling paging...\n");
    cr0 |= 0x80000000; // Enable paging!
    asm volatile("mov %0, %%cr0":: "r"(cr0));
	puts("switch_page_directory after enabling paging..\n");
}
...
page_directory_t* clone_directory(page_directory_t* src) {
	...
	//Then the physical of dir->tablesPhysical is:
	...
	for(i = 0; i < 1024; i++) {
		...
		if(kernel_directory->tables[i] == src->tables[i]) {
			...
			dir->tablesPhysical[i] = phys | 0x07; //0x07 = Present, read-write, user-mode
		}
	}
	return dir;
}
